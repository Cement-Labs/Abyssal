...
> [!NOTE]
> 由于 macOS 的限制，你可能无法感觉到特别明显的震动反馈。
...
由于 macOS 的限制，**Abyssal** 无法自动推断菜单栏上的可用收缩长度（全部长度减去应用菜单的长度）。所以，你可能需要在一些屏幕上手动调节收缩长度以获得最佳的使用体验。
...
为`自动闲置`功能设置一个超时限制，在达到此限制后自动关闭。可选时间范围为`5秒`到`10分钟`。
...
- **开**
  ...
  在 **Abyssal** 的各种行为间插入动画过渡。

- **关**

  减少动画的使用以降低 **Abyssal** 的占用。

<br />

> [!WARNING]
> 如果你正在使用一台 **Intel** 芯片的 Mac，则建议**开启**`减少动画`选项以保证 **Abyssal** 运行时不会出现过多卡顿。
...
- **关**
  ...
  **Abyssal** 将会在 macOS 启动后自动运行在后台。

- **开**

  你需要在每次重新启动 macOS 后手动运行 **Abyssal**。

<br />

### 附加功能：自动闲置

由于 macOS 的限制，**Abyssal** 无法得知你是否正在与位于**永远隐藏区域**或**隐藏区域**内的菜单图标交互。如果**自动隐藏**功能在鼠标移开菜单栏后立即隐藏这些图标，它们展开的菜单也会随之移开。因此，**Abyssal** 采用了一种能够很大限度避免此问题的变通方法。
...
在你**触发或取消自动隐藏或自动闲置**后， **Abyssal** 会产生一个 Haptic 触感反馈[^haptic_feedback_support_needed]。
...
> [!NOTE]
> 作为一个开源且免费的软件，**Abyssal** 暂无能力支付 [Apple 开发者账户](https://developer.apple.com/cn/help/account)的费用。因此，你无法从 App Store 中直接安装 **Abyssal。**你也许还需要允许 **Abyssal** 以未认证的应用身份运行[^open_as_unidentified]。
>
